...
export class OrderListComponent extends Component {
  ...
  handleShowModalTransfer(id) {
    modal.show(
      <FormUploadTransfer id={id} getData={this.getData}></FormUploadTransfer>,
      {
        width: "60%",
        style: {
          top: 20
        },
        title: <strong>Chọn bằng chứng thanh toán</strong>
      }
    );
  }
  render() {
          ...
          <Column
            ...
            render={value => {
              return value === "New" ? (
                ...
                <Tag color={COLOR_REJECT_CONTENT.Approved}>Đã thanh toán</Tag>
              ) : value === "Rejected" ? (
          ...
          <Column
            ...
            render={record => {
              if (record.status === "New") return <Tag>None</Tag>;
                ...
                return (
                  <p
                    onClick={() => this.showEvidence(record)}
          ...
          <Column
            ...
            render={record => {
              if (user && user.role === "CUSTOMER" && record.status === "New")
                return (
                  <Button
                    onClick={() => this.handleShowModalTransfer(record.id)}
                    icon="edit"
                    size="small"
                    type="primary"
                  >
